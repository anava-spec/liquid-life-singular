...
- Fixed a Direct Mail Tracker issue where a campaign linked to more than one audience only ever showed the first one — everywhere it appears (the campaign list, a campaign's detail page, and the exported CSV) now shows every linked audience. A campaign's detail page now lists its audiences as a table (name, type, total parcels, parcels in scope, active status, sortable by clicking a column), so it stays readable even with a dozen+ audiences on one campaign. The audience picker when creating a campaign is also cleaner: selected audiences move to the top of the list and highlight instead of showing a checkbox, and the field shows your picks as a short comma list instead of a wall of chips.
